...
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  ...
  --no-first-run &
```
Launching the binary directly (not via `open -a`) plus a scratch `--user-data-dir` reliably brings up the CDP endpoint within a few seconds. Warm up by navigating to `https://www.scouting.org/` first (`wait_until="load"`, then a few seconds' pause) before hitting a policy or PDF page — `wait_until="networkidle"` straight to one on a cold profile can time out entirely. That timeout was measured; the warm-up is kept on it. The "a cold profile has no Cloudflare clearance cookies" explanation this line used to give was assumed, not tested, and sits in the same class as the two corrected below.
...
- `download_pdf()` moved from `fetch_ranks.py` to `utils.py` and is now shared by both callers. It runs `fetch()` **inside the browser page** rather than using `context.request.get()`, which did not reliably work. That much was measured and the behaviour is retained on that observation alone. **The reason is not established, and the "Cloudflare clearance doesn't reach the CDN" explanation this line used to give is contradicted:** confirmed 2026-09-10, bare `curl` with no browser, no session and no clearance downloads a valid PDF from both hosts – the combined rank requirements PDF from `www.scouting.org` (1,190,527 bytes, 15 pages) and the Charter and Bylaws from `filestore.scouting.org` (431,902 bytes, 28 pages), 3/3 attempts each, `content-type: application/pdf`. So whatever made `context.request.get()` fail, it was not the CDN refusing an uncleared client.
...
**One confirmed instance means read the whole surrounding section — and the code, not just the docs.** Being pointed at a single false line here turned up three more in the two paragraphs around it, and then eight more in `scraper/`: module docstrings, inline comments, the `--cdp-url` help text, and a **user-facing error message** telling the operator that "scouting.org Cloudflare Enterprise blocks all automated downloads." Folklore in a docstring or an error string is worse than folklore in a playbook, because it is read at the moment someone is deciding whether a source is reachable. All corrected 2026-09-10, comments and strings only, no behaviour changed.

The strongest case was `download_pdf()`, which explained itself with "`context.request.get()` does not reliably pass Cloudflare clearance to the scouting.org CDN." That is not merely unestablished, it is **contradicted**: bare `curl` — no browser, no session, no clearance — downloads valid PDFs from both hosts, 3/3 (`Scouts-BSA-Rank-Requirements.pdf`, 1,190,527 bytes / 15 pages, `www.scouting.org`; `2025_Charter_Bylaws.pdf`, 431,902 bytes / 28 pages, `filestore.scouting.org`). The observation underneath it is real — one call failed, the other worked — so the in-page `fetch()` is retained on that observation alone, with the reason marked as not established.
